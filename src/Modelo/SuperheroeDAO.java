...
public class SuperheroeDAO {
    ...
    public void ListarSuperheroe (Connection conn, JComboBox nom_superheroe){
     ...
     //al final no usamos "insertarHeroe"
     public String insertarHeroe(Connection conn, JComboBox lst_AgregarHeroe){
        PreparedStatement pst;
        String tabla = "nuevoheroe";        
        String sql = "select nue_heroe from " +tabla;
        try {
            pst = conn.prepareStatement(sql);
            ResultSet result = pst.executeQuery();
            while (result.next()) {             
                
                lst_AgregarHeroe.addItem(result.getString("nue_heroe"));
            }
            } catch (SQLException | NullPointerException e) {
            System.out.println("No se pudo listar el nombre del superheroe" + e.getMessage());
        }   
